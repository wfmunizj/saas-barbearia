...
import { ArrowLeft, Scissors, DollarSign, CheckCircle, XCircle, Clock, Percent } from "lucide-react";
...
export default function BarberSummary() {
    ...
    return (
      <DashboardLayout>
        <div className="text-center py-16 text-muted-foreground">
          ...
          <Button variant="outline" className="mt-4" onClick={() => navigate("/barbeiros")}>
            Voltar
          </Button>
        </div>
      </DashboardLayout>
  ...
  const pendingCommissionIds = commissions
    ?.filter((c: any) => !c.paid && c.barberId === barberId)
    .map((c: any) => c.id) ?? [];
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="icon" onClick={() => navigate("/barbeiros")}>
            <ArrowLeft className="h-5 w-5" />
          </Button>
          <div>
            <h1 className="text-2xl font-bold">Resumo — {data.barber.name}</h1>
            <p className="text-muted-foreground text-sm">
              Desempenho e comissões do barbeiro
            </p>
          </div>
        </div>
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-2">
                {commissions?.filter((c: any) => !c.paid && c.barberId === barberId).map((c: any) => (
